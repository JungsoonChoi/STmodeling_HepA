################################################
# Modeling code for Poisson with Type 1 & Type 3
...
#######################################
# 2. Poisson with Type1 : NIMBLE code
#######################################
model.ID<-"Poi with Type1"
...
Model <- nimbleCode({ 
  for(i in 1:N.site) {
    for(j in 1:N.time) {
      y[i,j] ~ dpois(mu[i,j])
      log(mu[i,j]) <- beta0 + beta[1] * age[i,years[j]] + beta[2]*fore[i,years[j]] + beta[3]*edu[i]+beta[4]*income[i,years[j]] + beta[5]*sewage[i,years[j]] + beta[6]*water[i,years[j]] + beta[7]*birthrate[i,years[j]] + beta[8]*male[i,years[j]] + beta[9]*doctor[i,years[j]]+ alpha[1] * temp[i,j]+ alpha[2]* precip[i,j]+alpha[3]*humid[i,j]+log(popden[i,years[j]]) 
    }}
...
MCMCconfig <- configureMCMC(nimbleModel,monitors=c("beta0","beta","alpha","mu")) 
...
model.ID<-"Poi with Type1_residual"
...
inits <- list(alpha=0,u=rep(0,N.site),v=rep(0,N.site),t=rep(0,N.time),eta=rep(0,N.time),sigma.v=0.1,sigma.u=0.1,sigma.t=0.1,sigma.eta=0.1,sigma.res=0.1)
...
Model <- nimbleCode({ 
  for(i in 1:N.site) {
    for(j in 1:N.time) {
      y[i,j] ~ dpois(mu[i,j])
      log(mu[i,j]) <- beta0 + beta[1] * age[i,years[j]] + beta[2]*fore[i,years[j]] + beta[3]*edu[i]+beta[4]*income[i,years[j]] + beta[5]*sewage[i,years[j]] + beta[6]*water[i,years[j]] + beta[7]*birthrate[i,years[j]] + beta[8]*male[i,years[j]] + beta[9]*doctor[i,years[j]]+ alpha[1] * temp[i,j]+ alpha[2]* precip[i,j]+alpha[3]*humid[i,j]+log(popden[i,years[j]])+ST[i,j]+err[i,j] 
...
nimbleData <- list(y=y,age=age,popden=popden, fore=fore,edu=edu,income=income,sewage=sewage,water=water,birthrate=birthrate,male=male,doctor=doctor,temp=temp,precip=precip,humid=humid,ST=ST.new)
Consts <- list(N.site=250,N.time=209,years=years)
inits <- list(beta0=-0.1,beta=rep(0,9),alpha=rep(0,3),sigma.err=0.1,err=matrix(0,nrow=N.site,ncol=N.time))
nimbleModel <- nimbleModel(code = Model, name = "nimbleModel",constants = Consts, data = nimbleData, inits = inits)
MCMCconfig <- configureMCMC(nimbleModel,monitors=c("beta0","beta","alpha","sigma.err","err","mu")) 
nbMCMC <- buildMCMC(MCMCconfig)
Cnb <- compileNimble(nimbleModel)
CnbMCMC <- compileNimble(nbMCMC, project = nimbleModel)
