################################################
# Modeling code for ZIP with Type 2
...
#######################################
# 2. ZIP with Type2 : NIMBLE code
#######################################
model.ID<-"ZIP with Type2"
...
inits <- list(beta0=-0.1,gamma0=-0.1,beta=rep(0,9),alpha=rep(0,3),gamma=rep(0,9),delta=rep(0,3),
                u=rep(0,N.site),v=rep(0,N.site),t=rep(0,N.time),eta=rep(0,N.time),sigma.v=0.1,sigma.u=0.1,sigma.t=0.1,sigma.eta=0.1,
                zr=structure(.Data=rep(0, N.site*N.time), .Dim=c(N.site,N.time)))

nimbleModel <- nimbleModel(code = Model, name = "nimbleModel",constants = Consts, data = nimbleData, inits = inits)
MCMCconfig <- configureMCMC(nimbleModel,monitors=c("beta0","gamma0","beta","alpha","gamma","delta","sigma.u","sigma.v","sigma.t","sigma.eta","u","v","t","eta","mu")) 
